function [ model, metric ] = caploss_fm( training, validation, pars )
% with only capped $\ell_1$-norm loss.
        ...
        for t=1:epoch
            ...
            for j=1:num_sample
                ...
                if strcmp(task, 'regression')
                    ...
                    X(nz_idx) = 1;
                    y = Y_train(j,:);
                    y_predict = w0 + sum(W(nz_idx)) + sum(V(nz_idx(1),:).*V(nz_idx(2),:));
                else
                    X = X_train(j,:);
                    y = Y_train(j,:);
                    nz_idx = find(X);
...
%                 y_predict = w0 + W(nz_idx)*X(nz_idx)';
                ...
                if strcmp(task, 'regression')
                    ...
                    loss = loss + err^2;
                    abserr = abs(err);
                    if abserr > epsilon1 && abserr < epsilon2
...
%                         w0_ = learning_rate / (t0) * err / abserr;
%                         w0 = w0 - w0_;
%                         W_ = learning_rate / (t0) * (err *X(nz_idx) / abserr + 2 * reg * W(nz_idx));
%                         W(nz_idx) = W(nz_idx) - W_;
%                         V_ = learning_rate / (idx + t0) * (err / abserr * (repmat(X(nz_idx)',1,factors_num).*(repmat(X(nz_idx)*V(nz_idx,:),length(nz_idx),1)-repmat(X(nz_idx)',1,factors_num).*V(nz_idx,:))) + 2 * reg * V(nz_idx,:));
%                         V(nz_idx,:) = V(nz_idx,:) - V_; 
                    end
                end
